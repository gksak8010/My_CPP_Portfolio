...
void BookContainer::displayRentalBooks() const
{
	std::cout << "* 반납 예정 (대여중) 도서 리스트 *\n";
	for (int i = 0; i < index; i++)
	{
		if (p[i]->isRentalCheck())
			p[i]->displayBookInfo();
	}
}

void BookContainer::displayAvailableBooks() const
{
	std::cout << "* 대여 가능 도서 리스트 *\n";
	for (int i = 0; i < index; i++)
	{
		if (!p[i]->isRentalCheck())
			p[i]->displayBookInfo();
